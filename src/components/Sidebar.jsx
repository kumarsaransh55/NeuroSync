...
const mainMenuItems = [
    ...
    { name: 'Tasks', icon: CheckSquare, badge: '12+' },
...
export default function Sidebar() {
    return (
        <aside className="w-[240px] bg-white h-screen flex flex-col border-r border-[var(--color-border-color)] pb-6 shrink-0 transition-all duration-300 relative z-20 shadow-[var(--shadow-card)]">
            {/* Brand */}
                    ...
                    <ul className="space-y-1">
                        {mainMenuItems.map((item) => (
                            <li key={item.name}>
                                <a
                                    href="#"
                                    className={`flex items-center justify-between py-3 px-6 transition-all duration-200 ${item.active
                                ...
                                >
                                    <div className="flex items-center gap-3 font-medium text-[14px]">
                                        <item.icon size={18} className={item.active ? 'text-[var(--color-brand-end)]' : 'text-[var(--color-text-muted)]'} strokeWidth={2} />
                                        {item.name}
                                    </div>
                                    {item.badge && (
